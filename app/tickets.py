...
from reportlab.lib.pagesizes import A4
from reportlab.lib.units import mm
from reportlab.lib.utils import ImageReader   # ← FIX: needed to wrap BytesIO for drawImage
from reportlab.pdfgen import canvas as rl_canvas
...
def qr_image_reader(data: str) -> ImageReader:
    """
    Return QR code as a ReportLab ImageReader.
    FIX: ReportLab's drawImage() does NOT accept a raw BytesIO —
    it must be wrapped in ImageReader so it can read dimensions + pixels.
    """
    buf = io.BytesIO(qr_bytes(data))
    buf.seek(0)
...
def _draw_ticket_page(
    ...
    *,
    ...
    # FIX: Use ImageReader wrapper instead of passing BytesIO directly to drawImage
    qr_reader = qr_image_reader(verify_url)
...
def generate_pdf_tickets(
    ...
    Returns (pdf_path, ticket_records) where ticket_records is a list of
    { tier_name, ticket_code, verify_url, quantity_index } dicts — one entry
    per individual ticket (so 2x VIP = 2 records).
    """
    pdf_path = TICKETS_DIR / f"{order.uuid}.pdf"
    event_date_str = _fmt_order_date(order)
